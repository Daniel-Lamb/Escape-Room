...
- **Timer:** 60:00 in-game countdown in the HUD. Wrong puzzle inputs never cost time; only hints do. Amber under 10:00, pulsing red under 5:00.
- **Gus, the companion (franchise system):** the hint system is embodied in **Gus** — a persistent helper character who appears in every escape room this series ships, changing form to match the theme. In *The Pilgrim's Road* he is **Sir Gus**, the ghost of a Vayne Keep knight who owed Brother Edmund a debt and now haunts the walls, nudging prisoners along the pilgrim's road. He floats at the corner of every scene (gentle bob + spectral glow); clicking him opens his dialog: portrait, a line of period banter, and the hint ladder for the current room.
  - **Hint tiers (per room):** ① **Nudge** (−1:00) — points at the right objects, never the method. ② **Strong hint** (−2:00) — gives the method, not the values. ③ **The answer** (−4:00) — Gus spells out the exact solution. Tiers unlock strictly in order; once paid for, a tier stays **re-readable free forever** in Gus's dialog.
  - **Re-theming contract:** Gus's identity lives entirely in `js/gus.js` — `{ name, epithet, form, portrait SVG, voice lines }`. The engine consumes only that interface, so an animal-kingdom room can swap in Gus-the-monkey (same name, new form, new lines) without touching engine code. His name is always Gus.
- **Inventory:** bottom tray; items fly from scene to tray with spring easing. Click an item to **hold** it, then click a scene hotspot to **use** it there, or click a second inventory item to **combine** (registered pair recipes: flint + candle → lit candle; recipe halves → full recipe; draught + meat → drugged meat).
- **Pilgrim's Journal:** rooms are linear (no backtracking), so a HUD Journal auto-logs every examined clue verbatim (verses, roster, recipe, chronicle page) and auto-sketches every **sun-mark** with its ray count and letter the moment it is examined. Each sun-mark sits on or beside a mandatory puzzle object, so it cannot be missed — no pixel-hunting, no memory tax, and the Room 7 meta-puzzle is fully solvable from the journal alone.
- **No lockouts:** every puzzle state is resettable or (Room 6) mathematically always solvable. Wrong answers give a diegetic "clunk/hiss/discord" and reset. If the timer expires, the defeat screen offers **"Rise again"** — retry from the current chamber with all items/progress kept and a fresh 15:00 (a full restart is also offered).
- **Tech:** vanilla JS ES modules, no build step, static site (GitHub Pages). Scenes are layered hand-authored SVG (layer order: stone architecture → furniture/props → light effects last) with a CSS animation library (`torch-flame`, `glow`, `moonbeam`, `fog`, `sway`, `spin`...). Bell tones, sfx, and wind/drip ambience are synthesized with Web Audio oscillators — zero asset files. Room crossfades, spring-eased modals, typewriter narrative, drifting ember particles. Autosave to localStorage on every meaningful action.
